...
This major release uses common v5 (Kubernetes 1.31+, Helm 3.18+). The library
now creates a dedicated service account and disables automatic token mounting
by default. Review rawResources: their manifest belongs under `manifest`,
replacing the old `spec` wrapper. Run `helm dependency build` from Chart.lock;
the obsolete bundled common v4 copy has been removed.
